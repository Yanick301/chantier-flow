...
router.get('/:id', (req: AuthRequest, res: Response) => {
  try {
    ...
    const caisses = db.prepare('SELECT * FROM caisses WHERE chantier_id = ?').all(req.params.id);
    const controleurs = db.prepare(`
...
router.post('/', roleMiddleware('president', 'controleur'), (req: AuthRequest, res: Response) => {
  try {
    ...
    if (controleurs && Array.isArray(controleurs)) {
      const stmt = db.prepare('INSERT OR IGNORE INTO chantiers_controleurs (chantier_id, controleur_id) VALUES (?, ?)');
      for (const ctrlId of controleurs) {
        stmt.run(chantierId, ctrlId);
      }
    }

    logAudit(req.user!.id, 'creation', 'chantier', chantierId,
...
router.put('/:id', roleMiddleware('president', 'controleur'), (req: AuthRequest, res: Response) => {
  try {
    ...
    const existing = db.prepare('SELECT * FROM chantiers WHERE id = ?').get(req.params.id) as any;
    if (!existing) {
      res.status(404).json({ error: 'Chantier non trouvé' });
      return;
    }
    ...
    if (controleurs && Array.isArray(controleurs)) {
      db.prepare('DELETE FROM chantiers_controleurs WHERE chantier_id = ?').run(req.params.id);
      const stmt = db.prepare('INSERT OR IGNORE INTO chantiers_controleurs (chantier_id, controleur_id) VALUES (?, ?)');
      for (const ctrlId of controleurs) {
        stmt.run(req.params.id, ctrlId);
      }
    }

    logAudit(req.user!.id, 'modification', 'chantier', parseInt(req.params.id),
...
router.delete('/:id', roleMiddleware('president'), (req: AuthRequest, res: Response) => {
  try {
    const db = getDb();
    db.prepare('UPDATE chantiers SET statut = \'termine\', date_modification = datetime(\'now\') WHERE id = ?').run(req.params.id);

    logAudit(req.user!.id, 'cloture', 'chantier', parseInt(req.params.id),
